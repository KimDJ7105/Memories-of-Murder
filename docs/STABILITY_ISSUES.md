...
## 4. 다중 Room 동시 테스트 (수정 불필요)

**테스트**

두 개의 방(A, B)을 거의 동시에 만들고(방 코드 충돌 가능성을 최대화하기 위해 백투백으로 생성), 각각 3명씩 넣고 동시에 `start_game`을 보내 서버의 단일 `io_context` 스레드에서 두 방의 메시지 처리가 실제로 인터리빙되게 만든 뒤:

1. 방 코드가 서로 다른지 확인.
2. 각 방의 `room_update` 브로드캐스트에 상대 방 플레이어 이름이 섞여 들어오는지 확인.
3. 방 A를 1번 항목의 크래시 시나리오(범행 제출 직후 AI 응답을 기다리지 않고 전원 접속 종료)로 강제 종료시키고, 그 동안과 그 이후 방 B가 계속 정상 응답하는지 확인.

**결과**

- 방 코드 충돌 없음.
- 방 A의 브로드캐스트에 방 B 플레이어 이름이, 방 B의 브로드캐스트에 방 A 플레이어 이름이 전혀 나타나지 않음 — `RoomManager`의 방별 `RoomSender`가 의도대로 완전히 격리됨.
- 방 A를 AI 콜백 진행 중에 강제로 파괴한 뒤에도 방 B는 계속 정상적으로 `error` 응답을 보내는 등 전혀 영향받지 않음. 서버 프로세스도 동일 PID로 계속 생존.

`RoomManager` 설계(방마다 독립된 `sessions`/`RoomSender`/`GameRoom`)가 실제로 격리를 보장하고 있음을 확인 — 별도 조치 불필요.

---

## 5. 탐정 Disconnect / Ollama 연결 실패 — 스캔 결과 (수정 불필요)

**탐정 Disconnect (현재 턴인 사람이 끊기는 경우)**

`handle_disconnect`의 `Investigation` 분기(`was_current` 체크)를 코드로 확인한 뒤, 실제로 3인 방에서 현재 턴인 탐정의 소켓을 추리 제출 없이 그냥 닫아봤다. 남은 두 플레이어에게 곧바로 새로운 `investigation_turn_start`(다음 탐정)가 도착함을 확인 — 게임이 멈추지 않고 정상적으로 다음 차례로 넘어간다. 범인 Disconnect는 이미 Phase 2~3 개발 중 여러 번 코드로 다뤄졌고(라운드 강제 종료, `docs/PROTOCOL.md` 참고) 이번 재검증에서도 별도 문제 없음.

**Ollama 연결 실패**

Ollama 프로세스를 완전히 종료한 상태에서 범행을 제출해봤다. `OllamaClient`의 TCP 연결이 즉시 거부되어(포트에 아무도 없으므로 타임아웃까지 기다리지 않고 바로 실패) `crime_score_revealed`에 fallback 기본값인 `50`점이 즉시 반영됐고, 게임은 멈추거나 죽지 않고 정상적으로 `Investigation` 상태로 진행됐다. Phase 2에서 이미 구현해 둔 `fallback_evaluation`/`failure_feedback` 경로가 여전히 잘 동작함을 재확인 — 별도 조치 불필요.
