...
## 1-3. Windows 로케일 오류 메시지의 잘못된 UTF-8이 JSON 직렬화에서 서버 전체를 죽임

> 이 항목은 Phase 5 종료 후, 항목별 점수 breakdown 기능(feature-score-breakdown)을 개발하며 AI 실패 경로를 다시 테스트하던 중 우연히 재현되어 이 문서에 추가되었다.

### 구조적 배경

`OllamaClient::fail()`은 연결/쓰기/읽기 단계에서 에러가 나면 `boost::system::error_code::message()`를 그대로 실패 사유 문자열로 만들어 콜백에 넘긴다. 이 문자열은 결국 `CrimeEvaluation::evaluation`(예: `"AI 평가에 실패하여 ... (" + reason + ")"`)에 섞여 들어가고, 최종적으로 `Session::send()`가 `nlohmann::json::dump()`로 직렬화해서 클라이언트에 보낸다. `error_code::message()`는 Windows에서 내부적으로 `FormatMessageA`를 호출하는데, 이건 **시스템의 ANSI 코드페이지(한글 Windows에서는 CP949)로 인코딩된 문자열**을 돌려준다 — UTF-8이 아니다. `nlohmann::json::dump()`는 기본적으로 UTF-8 유효성을 검사하고, 유효하지 않으면 예외(`type_error.316`)를 던진다.

### 테스트와 결과

항목별 점수 breakdown 기능을 실제 서버로 테스트하던 중, Ollama를 종료한 상태에서 범행을 제출했더니 서버가 다음 로그를 남기고 완전히 죽었다.

```
[game_server] fatal: [json.exception.type_error.316] invalid UTF-8 byte at index 34: 0xB4
```

Phase 5에서 이미 "Ollama 연결 실패"를 테스트했었는데(2-4 항목) 그때는 크래시가 없었다 — 그때 우연히 발생한 OS 에러가 영어(ASCII) 메시지였을 뿐, 이번엔 다른 종류의 네트워크 에러(`WinError 64`, "지정된 네트워크 이름은 더 이상 사용할 수 없습니다")가 발생해서 한글 메시지가 반환됐고, 그게 처음으로 이 잠재된 버그를 실제로 건드린 것이었다. 즉 **이 버그는 Phase 5 때부터 이미 존재했지만, 그때는 운 좋게 트리거되지 않았을 뿐**이다.

이 예외는 어디서도 잡히지 않고 `io_context::run()` 바깥까지 전파되어 `main()`의 최상위 `try/catch`에 걸려 프로세스를 종료시켰다 — 크래시를 일으킨 방뿐 아니라 그 순간 서버에 떠 있던 다른 모든 방까지 함께 죽는, 1-1과 동일한 심각도의 문제였다.

### 조치

`nlohmann::json::dump()`를 호출하는 세 곳 전부 기본(예외를 던지는) 핸들러 대신 `error_handler_t::replace`(잘못된 UTF-8 시퀀스를 U+FFFD로 치환하고 절대 예외를 던지지 않음)를 쓰도록 변경했다.

- `server/src/Session.cpp` — 모든 클라이언트 전송의 최종 관문. 가장 중요한 수정.
- `server/src/AiTestLog.cpp` — `AI_TEST_MODE`가 켜져 있으면 `Session::send`보다도 먼저 같은 문자열을 파일에 기록하므로 동일하게 취약.
- `server/src/OllamaClient.cpp` — Ollama에 보내는 요청 본문(현재는 항상 유효한 UTF-8만 들어가지만, 방어적으로 동일하게 처리).

### 재검증 결과

Ollama를 완전히 종료한 상태에서 같은 시나리오(범행 제출 → AI 실패 → 폴백)를 다시 실행 — 서버가 죽지 않고 폴백 점수(항목별 절반 점수 breakdown, 합계 51점)가 정상적으로 `round_result`에 담겨 전달됨을 확인했다. 서버 프로세스도 테스트 전후로 동일한 PID를 유지했다.
...
1부에서 발견된 세 실제 버그(1-1, 1-2, 1-3)는 전부 **AI 비동기 호출 자체의 처리량이나 큐잉 문제가 아니라, 콜백/상태의 수명 관리 또는 예외/인코딩 처리 문제**였다. Job Queue나 별도 Worker Thread를 추가한다고 해서 이 버그들이 저절로 해결되지는 않았을 것이다 — 오히려 워커 스레드가 게임 스레드와 물리적으로 분리돼 있었다면, `this` 캡처 문제에 스레드 간 데이터 경합까지 겹쳐 디버깅이 훨씬 더 어려운 문제가 됐을 가능성이 크다. 특히 1-3은 "AI 응답을 성공적으로 처리하는 경로"가 아니라 "AI 실패를 안전하게 처리하려던 경로 자체"에 있던 결함이라는 점에서, 실패 처리 코드도 성공 경로만큼 꼼꼼히 검증해야 한다는 교훈을 남긴다.
